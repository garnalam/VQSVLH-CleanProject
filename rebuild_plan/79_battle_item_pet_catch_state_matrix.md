...
| Petstate click/input | Source `petstate.ui` row containers are spaced every 15 px from y `86`, not like `choice.ui`. | PORTED: rebuild has petstate-specific row hitbox and smoke `battle_p5_click_reserve_success`. Live checkpoint `battle_elder_command_ui` now seeds a reserve pet so P5 can be tested directly. Renderer now follows source `petstate.ui` coordinates for row bars, selected sprite area, stars, stat bars, and action/back labels; still PARTIAL for full `ao` widget runtime/equipment/evolution fields. |
